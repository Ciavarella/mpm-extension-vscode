...
export default class MusicPerMinute {
  ...
  public async requestNewToken(): Promise<void> {
    const refreshKey = this.ctx.globalState.get('refresh_token');
  ...
  public async getUserSettings(): Promise<void> {
    ...
    if (user !== undefined) {
      ...
      if (settings !== null) {
        ...
        this.ctx.globalState.update('hardmode', this.hardMode);
        this.ctx.globalState.update('keypress', this.keypressTime);
      } else {
        this.keypressTime = 1;
        this.hardMode = false;
      }
